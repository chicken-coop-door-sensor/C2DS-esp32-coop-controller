...
#define CONNECTION_TIMEOUT pdMS_TO_TICKS(30000)  // 30 seconds timeout
...
void wifi_init_sta(void) {
    ...
    ESP_LOGI(TAG, "Randomized MAC Address: %02x:%02x:%02x:%02x:%02x:%02x", mac[0], mac[1], mac[2],
             mac[3], mac[4], mac[5]);
    ...
    ESP_LOGI(TAG, "attempting login for ssid: %s password: %s", CONFIG_WIFI_SSID,
    ...
    EventBits_t bits = xEventGroupWaitBits(s_wifi_event_group, WIFI_CONNECTED_BIT | WIFI_FAIL_BIT,
                                           pdFALSE, pdFALSE, CONNECTION_TIMEOUT);
    ...
    if (bits & WIFI_CONNECTED_BIT) {
        ...
        ESP_LOGE(TAG, "Connection timed out. Rebooting...");
        esp_restart();  // Reboot the device
    }
